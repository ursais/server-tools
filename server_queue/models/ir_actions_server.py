...
class IrActionsServer(models.Model):
    ...
    enqueue_server_action = fields.Boolean(
        ...
        help="If enabled, this server action will be queued instead of executed directly."
    )
    ...
    def run(self):
        ...
        for action in self:
            if action.enqueue_server_action:
                ...
                active_ids = env.context.get("active_ids") or ([active_id] if active_id else [])
                ...
                for res_id in active_ids:
                    env["server.queue"].create({
                        ...
                        "kwargs": str({k: v for k, v in env.context.items() if not k.startswith('active_')}),
                    })
                    _logger.info("Action %s deferred and enqueued for record %s", action.name, res_id)
                return True
